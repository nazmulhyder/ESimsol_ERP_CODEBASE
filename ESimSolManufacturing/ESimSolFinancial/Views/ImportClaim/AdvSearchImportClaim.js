var IICService = angular.module('importClaim.service', ['ms.service', 'ngAnimate', 'ui.bootstrap']);
IICService.service('importClaimservice', function ($uibModal) {
    ...
    this.Instance = function (modalProperty) {
        var modalInstance = {
            ...
            size: 'md',
            templateUrl: _sBaseAddress + '/ImportClaim/AdvSearchImportClaim',
            controller: 'ModalImportClaimAdvanceSearchCtrl',
            ...
            resolve: {
                obj: function () {
                    return {
                        ...
                        ClaimTypes: (modalProperty.ClaimTypes == undefined) ? [] : modalProperty.ClaimTypes
                    };
                }
        ...
        modalInstance.controller = function ($http, $scope, $log, $uibModalInstance, uiGridConstants, msModal, icsMethod, obj) {
            ...
            $scope.PickerTitle = "Import Claim";
                ...
                $http.post(_sBaseAddress + '/Contractor/ContractorSearchByNameType', $.param(oContractor), config).then(
                                    function (response) {
                                        ...
                                        var modalObj = {
                                            ...
                                            appcontroller: 'ImportInvoiceClaimController',
                                            objs: results,
                                            multiSelect: true,
            ...
            $scope.ClaimDateChange = function () {
                $scope.ClaimDateEndDisabled = icsMethod.IsDateBoxdisabled($scope.cboClaimDate);
            }
            ...
            $scope.search = function () {
                ...
                if ($scope.cboClaimDate == 0 && ($scope.ImportInvoiceNo == undefined ? "" : $scope.ImportInvoiceNo) == "" && ($scope.ImportLCNo == undefined ? "" : $scope.ImportLCNo) == "" && ($scope.ContractorIDs == undefined ? "" : $scope.ContractorIDs) == "" && $scope.cboClaimType == 0) {
                    ...
                    alert("Invalid Searching Criteria!!");
                    $scope.lblLoadingMessage = false;
                    return false;
                }

                var sTempString = $scope.cboClaimDate + '~' + $scope.ClaimDateStart + '~' + $scope.ClaimDateEnd + '~' + ($scope.ImportInvoiceNo == undefined ? "" : $scope.ImportInvoiceNo) + '~' + ($scope.ImportLCNo == undefined ? "" : $scope.ImportLCNo) + '~' + ($scope.ContractorIDs == undefined ? "" : $scope.ContractorIDs) + '~' + sessionStorage.getItem('BUID') + '~' + ($scope.cboClaimType == undefined ? 0 : $scope.cboClaimType);
                
                sessionStorage.setItem("sTempString", sTempString);
                var config = { headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;' } };
                $http.get(_sBaseAddress + '/ImportClaim/AdvSearch', { params: { sTemp: sTempString } }, config).then(
                            function (response) {
                                $scope.lblLoadingMessage = true;
                                var results = jQuery.parseJSON(response.data);
                                if (results.length > 0) {
            ...
            $scope.reset = function () {
                ...
                $scope.ClaimTypes = obj.ClaimTypes;
                $scope.ImportInvoiceNo = $scope.ImportLCNo = $scope.CPName = "";
                $scope.ContractorIDs = "";
                $scope.ContractorName = "";
                $scope.cboClaimDate = $scope.CompareOperators[0].id;
                $scope.ClaimDateEndDisabled = true;
                $scope.ClaimDateStart = $scope.ClaimDateEnd = icsdateformat(new Date());
            }
            $scope.reset();
