...
namespace ESimSolFinancial.Controllers
{
    public class PurchaseBillController : Controller
    {
        ...
        public ActionResult ViewPurchaseBills(int buid, int menuid)
        {
        ...
        public ActionResult ViewPurchaseBill(int nid, decimal ts)
        {
            _oClientOperationSetting = new ClientOperationSetting();
            _oPurchaseInvoiceDetails = new List<PurchaseInvoiceDetail>();
            ...
            if (nid > 0)
            {
                oPurchaseInvoice = oPurchaseInvoice.Get(nid, (int)Session[SessionInfo.currentUserID]);
                 _oPurchaseInvoiceDetails = PurchaseInvoiceDetail.GetsByPurchaseInvoiceID(nid, (int)Session[SessionInfo.currentUserID]);
                 oPurchaseInvoice.PurchaseInvoiceDetails = _oPurchaseInvoiceDetails.OrderBy(x => x.PurchaseInvoiceDetailID).ToList();
            }
            else
            {
        ...
        public ActionResult ViewLandingCost(int nid, decimal ts)
        {
            Company oCompany = new Company();
            oCompany = oCompany.Get(1, (int)Session[SessionInfo.currentUserID]);
            PurchaseInvoice oPurchaseInvoice = new PurchaseInvoice();
            ...
            {
                if ((EnumPInvoiceType)oItem.id == EnumPInvoiceType.Standard)
                {
        ...
        public JsonResult GetProducts(Product oProduct)
        {
            ...
            try
            {
                ...
                {
                    string sSQL = "SELECT * FROM View_Product AS HH WHERE HH.Activity = 1 AND ISNULL(HH.ProductName,'')+ISNULL(HH.ProductCode,'') LIKE '%" + oProduct.ProductName + "%' AND  HH.ProductCategoryID IN (SELECT PC.ProductCategoryID FROM [dbo].[FN_GetProductCategoryByBUModuleAndUser](" + (int)Session[SessionInfo.currentUserID] + "," + (int)EnumModuleName.PurchaseInvoice + "," + (int)EnumProductUsages.Regular + "," + oProduct.BUID + ") AS PC) ORDER BY HH.ProductName";
            ...
            try
            {
                if (oPurchaseInvoice.RefID > 0)
                {
                    ...
                    foreach (PurchaseOrderDetail oItem in oPurchaseOrderDetails)
                    {
                        ...
                        oPurchaseInvoiceDetail.VehicleModelID = oItem.VehicleModelID;
                        oPurchaseInvoiceDetail.ModelNo = oItem.ModelShortName;
                        oPurchaseInvoiceDetail.ModelShortName = oItem.ModelShortName;
        ...
        public JsonResult GetsCostHeads(ChartsOfAccount oChartsOfAccount)//for landing Cost entry
        {
            ...
            try
            {
                string sSQL = "SELECT * FROM View_ChartsOfAccount WHERE AccountType = " + (int)EnumAccountType.Ledger+ " AND ComponentID = 6";
        ...
        private string GetSQL(string sSearchingData, int nBUID)
        {
            ...
            int nInvoicePaymentMode = Convert.ToInt32(sSearchingData.Split('~')[17]);
            ...
            sReturn = sReturn + " RefType IN (" + (int)EnumInvoiceReferenceType.Open + "," + (int)EnumInvoiceReferenceType.PO + ")";
            //}
            //else
            //{
            //    Global.TagSQL(ref sReturn);
        ...
        public ActionResult PrintPurchaseInvoice(int id)
        {
            ...
            _oPurchaseInvoice.PurchaseInvoiceDetails = _oPurchaseInvoice.PurchaseInvoiceDetails.OrderBy(x => x.PurchaseInvoiceDetailID).ToList();

            List<ApprovalHead> oApprovalHeads = new List<ApprovalHead>();
            string ssql = "select * from ApprovalHead where ModuleID = "+(int)EnumModuleName.PurchaseInvoice;
            ...
            rptPurchaseBill oReport = new rptPurchaseBill();
            byte[] abytes = oReport.PrepareReport(_oPurchaseInvoice, oCompany, oContractor, oBusinessUnit, oApprovalHeads);
