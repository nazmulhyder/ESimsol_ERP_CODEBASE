...
namespace ESimSol.Services.Services
{
    public class MailSetUpService : MarshalByRefObject, IMailSetUpService
    {
        ...
        private MailSetUp MapObject(NullHandler oReader)
        {
            ...
            oMailSetUp.LastMailTime = oReader.GetDateTime("LastMailTime");
            ...
            oMailSetUp.IsMail = oReader.GetBoolean("IsMail");
            oMailSetUp.ModuleType = (EnumModuleName)oReader.GetInt32("ModuleType");
            oMailSetUp.ModuleTypeInt = oReader.GetInt32("ModuleType");
        ...
        public MailSetUp GetByModule(int nModuleID, Int64 nUserId)
        {
            MailSetUp oMailSetUp = new MailSetUp();
            TransactionContext tc = null;
            try
            {
                ...
                IDataReader reader = MailSetUpDA.GetByModule(tc, nModuleID);
                NullHandler oReader = new NullHandler(reader);
                if (reader.Read())
                {
                    oMailSetUp = CreateObject(oReader);
