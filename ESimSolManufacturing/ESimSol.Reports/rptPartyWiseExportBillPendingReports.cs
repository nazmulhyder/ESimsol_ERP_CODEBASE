...
{
    public class rptPartyWiseExportBillPendingReports
    {
        #region Declaration
        Document _oDocument;
        ...
        public byte[] PrepareReport(ExportBillPendingReport oExportBillPendingReport, int nReportType)
        {
            ...
            _oExportBillPendingReports = oExportBillPendingReport.ExportBillPendingReports.OrderBy(c => c.ApplicantID).ToList();
        ...
        private void PrintBody()
        {
            ...
            if (_nReportType == 1)
            {
                ...
                foreach (ExportBillPendingReport oItem in _oExportBillPendingReports)
                {
                    ...
                    if (_nApplicantID != oItem.ApplicantID)
                    {
                        nMaxCountDuplicate = _oExportBillPendingReports.Where(item => item.ApplicantID == oItem.ApplicantID).Count();

                        _oFontStyle = FontFactory.GetFont("Tahoma", 8f, 1);
                        _oPdfPCell = new PdfPCell(new Phrase(oItem.ApplicantName, FontFactory.GetFont("Tahoma", 13f, iTextSharp.text.Font.BOLD)));
                        ...
                        _oPdfPCell = new PdfPCell(new Phrase("Bank", _oFontStyle)); //BankName_Nego
                        _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER; _oPdfPCell.BackgroundColor = BaseColor.LIGHT_GRAY; oPdfPTable.AddCell(_oPdfPCell);
                    ...
                    _nApplicantID = oItem.ApplicantID;
                }
                #region Grand Total
                _oFontStyle = FontFactory.GetFont("Tahoma", 8f, 1);
            ...
            else if (_nReportType == 3)
            {
                ...
                oPdfPTable.WidthPercentage = 100;
                oPdfPTable.HorizontalAlignment = Element.ALIGN_LEFT;
                oPdfPTable.SetWidths(new float[] { 35f, 200f, 200f, 200f, 100f, 100f, 100f, 150f, 80f, 150f, 150f });
            ...
            else if (_nReportType == 4)
            {
                ...
                foreach (ExportBillPendingReport oItem in _oExportBillPendingReports)
                {
                    ...
                    if (_nApplicantID != oItem.ApplicantID)
                    {
                        ...
                        _oPdfPCell = new PdfPCell(new Phrase("Receive from party", _oFontStyle)); //Receive from party ???
                        _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER; _oPdfPCell.BackgroundColor = BaseColor.LIGHT_GRAY; oPdfPTable.AddCell(_oPdfPCell);
