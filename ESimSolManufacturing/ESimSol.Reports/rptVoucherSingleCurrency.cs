...
{
    public class rptVoucherSingleCurrency
    {
        #region Declaration
        Document _oDocument;
        ...
        public byte[] PrepareReport(Voucher oVoucher, int nBUID, bool bLogoPrint)
        {
        ...
        private void PrintBody()
        {
            ...
            {
                _oPdfPCell = new PdfPCell(new Phrase("In Word : " + Global.DollarWords(_nSumDebitAmount), _oFontStyle));
                _oPdfPCell.Colspan = 2; _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.Border = iTextSharp.text.Rectangle.BOTTOM_BORDER | iTextSharp.text.Rectangle.LEFT_BORDER | iTextSharp.text.Rectangle.RIGHT_BORDER; _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);
            }

            Chunk oChunk = new Chunk();
            Phrase oPhrase = new Phrase();
            oChunk = new Chunk(_oVoucher.CUSymbol + " " + Global.MillionFormat(_nSumDebitAmount), _oFontStyle);
            ...
            oChunk = new Chunk();
            oPhrase = new Phrase();
            oChunk = new Chunk(_oVoucher.CUSymbol + " " + Global.MillionFormat(_nSumCreditAmount), _oFontStyle);
            oChunk.SetUnderline(0.7f, -3f);
            oChunk.SetUnderline(0.7f, -5f);
            oPhrase.Add(oChunk);
            _oPdfPCell = new PdfPCell(oPhrase);
