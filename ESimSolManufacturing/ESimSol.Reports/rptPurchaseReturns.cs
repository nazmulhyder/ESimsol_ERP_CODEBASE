...
{
    public class rptPurchaseReturns
    {
        #region Declaration
        Document _oDocument;
        ...
        double _nSumPurchaseReturnAmount = 0;
        string _sMessage = "";

        #endregion

        public byte[] PrepareReport(List<PurchaseReturn> oPurchaseReturns, Company oCompany, string sMessage)
        {
            _oPurchaseReturns = oPurchaseReturns;
            _oCompany = oCompany;
            _sMessage = sMessage;
            ...
            _oPdfPTable.SetWidths(new float[] { 
                                                30f,  //SL No,    
                                                45f, //PurchaseReturn No,
                                                50f, // PurchaseReturn DAte,
                                                140f, // Vendor,
                                                120f, // Store
                                                80f, //Challa no
                                                100f // Approve by
        ...
        private void PrintBody()
        {
            ...
            int nCount = 0; double nTotalPurchaseReturnAmount = 0; string sCurrencySymbol = "";
            _oFontStyle = FontFactory.GetFont("Tahoma", 6f, 0);
            foreach (PurchaseReturn oItem in _oPurchaseReturns)
            {
                nCount++;
                _oPdfPCell = new PdfPCell(new Phrase(nCount.ToString(), _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);

                _oPdfPCell = new PdfPCell(new Phrase(oItem.ReturnNo, _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);
