...
namespace ESimSol.Reports
{
    public class rptCashSheet
    {
        ...
        #endregion
        public byte[] PrepareReport(EmployeeSalary oEmployeeSalary, List<SalarySheetProperty> oSalarySheetPropertys)
        {
        ...
        private void PrintHeader()
        {
            ...
            oPdfPTableHeader.SetWidths(new float[] { 180f, 280f });
            ...
            _oPdfPCell = new PdfPCell(new Phrase("Cash Sheet"));
            _oPdfPCell.Colspan = 7;
            _oPdfPCell.Border = 0;
            _oPdfPCell.FixedHeight = 20;
        ...
        private void PrintBody()
        {
            ...
            foreach (RPTSalarySheet oItem in _oEmployeeSalarys)
            {
                if (oItem.CashAmount > 0)
                {
                    ...
                    if (oItem.BankAmount <= 0 && oItem.CashAmount > 0)
                    {
                        oItem.CashAmount -= ((_bHasOTAllowance) ? 0 : nOTAllowance);
                    }

                    nTotalAmount += oItem.CashAmount;
                    _oPdfPCell = new PdfPCell(new Phrase(oItem.CashAmount.ToString(), _oFontStyle));
                    _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);
            ...
            _oPdfPCell = new PdfPCell(new Phrase(nTotalAmount.ToString(), _oFontStyle));
