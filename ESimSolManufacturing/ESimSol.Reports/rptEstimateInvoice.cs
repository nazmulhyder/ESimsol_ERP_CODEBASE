...
{
    public class rptEstimateInvoice
    {
        #region Declaration
        Document _oDocument;
        ...
        public byte[] PrepareReport(ServiceInvoice oServiceInvoice, Company oCompany, List<SignatureSetup> oSignatureSetups)
        {
            ...
            this.PrintHeader("Repair Estimates");
            this.PrintBody();
        ...
        private void PrintBody()
        {
            ...
            this.AddCell(ref oSIDPdfPTable, "", "CENTER", 1, 0, 0);
            this.AddCell(ref oSIDPdfPTable, "", "CENTER", 1, 0, 1);
            this.AddCell(ref oSIDPdfPTable, "VAT(" + _oServiceInvoice.LaborCharge_Vat + "%)", "LEFT", 5, 0, 1);
            ...
            this.PrintRow("");
            this.PrintRow("");
            _oFontStyle = this.GetCustomFont_UNDERLINE("AudiType-Bold", 8); 
            PrintRow("*Conditions Apply ");
            this.PrintRow("");
            this.PrintRow("");
            _oFontStyle = this.GetCustomFont("AudiType-Bold", 7);
            PrintRow("Terms & Conditions:"); nCount = 0;
            foreach(ServiceInvoiceTerms oItem in _oServiceInvoice.ServiceInvoiceTermsList)
            {
                nCount++;
                PrintRow(nCount.ToString()+") "+oItem.Terms);
            }
            while (this.CalculatePdfPTableHeight(_oPdfPTable) < 680f)
            {
                this.PrintRow("");
            }
            _oSignatureSetups = new List<SignatureSetup>();
            SignatureSetup oSignatureSetup = new SignatureSetup(); oSignatureSetup.SignatureCaption = "Signature(Service Advisor)";  _oSignatureSetups.Add(oSignatureSetup);
            oSignatureSetup = new SignatureSetup(); oSignatureSetup.SignatureCaption = "Customer Signature";_oSignatureSetups.Add(oSignatureSetup);
             #region print Signature Captions
             if (_bIsCustomerCopy == false)
             {
                 
                 _oPdfPCell = new PdfPCell(ESimSolSignature.GetSignature(540f, (object)_oServiceInvoice, _oSignatureSetups, 0.0f)); _oPdfPCell.Colspan = _nColspan;
