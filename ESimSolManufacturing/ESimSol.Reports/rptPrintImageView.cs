...
{
    public class rptPrintImageView
    {
        #region Declaration
        int _nTotalColumn = 1;
        Document _oDocument;
        ...
        public byte[] PrepareReport(List<DispoProductionComment> oDispoProductionComments, Company oCompany)
        {
            _oDispoProductionComments = oDispoProductionComments;
            _oCompany = oCompany;
        ...
        {
            this.PrintHeaderDetail("Comment Images List", " ", " ");
            this.SetData();
        }
        #endregion
        ...
        private void SetData()
        {
            ...
            PdfPTable oPdfPTable = new PdfPTable(3);
            oPdfPTable.SetWidths(new float[] { 10f, 60f, 30f });

            foreach (DispoProductionComment oItem in _oDispoProductionComments)
            {
                oPdfPTable = new PdfPTable(2);
                oPdfPTable.SetWidths(new float[] { 30f, 70f });

                //ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Comment", 0, oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, true, 15, _oFontStyleBold);
                //oPdfPTable.CompleteRow();

                _oPdfPCell = new PdfPCell(new Phrase("Comment By: " + oItem.UserName, _oFontStyleBold)); _oPdfPCell.Border = 15;
                _oPdfPCell.MinimumHeight = 15; _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.VerticalAlignment = Element.ALIGN_TOP;
                _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);

                _oPdfPCell = new PdfPCell(new Phrase("Comment: " + oItem.Comment, _oFontStyleBold)); _oPdfPCell.Border = 15;
                _oPdfPCell.MinimumHeight = 15; _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.VerticalAlignment = Element.ALIGN_TOP;
                _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);
                oPdfPTable.CompleteRow();
                ESimSolItexSharp.PushTableInCell(ref _oPdfPTable, oPdfPTable, 0, _oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, false, true);
                //ESimSolItexSharp.SetCellValue(ref oPdfPTable, " ", 0, oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, false, 15, _oFontStyle);
                //oPdfPTable.CompleteRow();

                oPdfPTable = new PdfPTable(3);
                oPdfPTable.SetWidths(new float[] { 10f, 80f, 10f });

                #region Image
                foreach (var oAttachment in oItem.DispoProductionAttachments)
                {
                    oPdfPTable = new PdfPTable(3);
                    oPdfPTable.SetWidths(new float[] { 10f, 80f, 10f });

                    ESimSolItexSharp.SetCellValue(ref oPdfPTable, " ", 6, 0, Element.ALIGN_CENTER, BaseColor.WHITE, false, 0, _oFontStyle);
                    if (oAttachment.Image != null)
                    {
                        _oImag = iTextSharp.text.Image.GetInstance(oAttachment.Image, System.Drawing.Imaging.ImageFormat.Jpeg);
                        //_oImag.ScaleAbsolute(500f, 350f);
                        //_oPdfPCell = new PdfPCell(_oImag);
                        _oImag.ScaleAbsolute(205f, 55f);
                        _oPdfPCell = new PdfPCell(_oImag);
                        _oPdfPCell.Rowspan = 6;

                        _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                        _oPdfPCell.VerticalAlignment = Element.ALIGN_BOTTOM;
                        _oPdfPCell.PaddingTop = 30;
                        _oPdfPCell.Border = 0;
                        _oPdfPCell.MinimumHeight = 60;
                        oPdfPTable.AddCell(_oPdfPCell);
                    }
                    else
                    {
                        _oPdfPCell = new PdfPCell(new Phrase("", _oFontStyle)); _oPdfPCell.Rowspan = 6; _oPdfPCell.Border = 0;
