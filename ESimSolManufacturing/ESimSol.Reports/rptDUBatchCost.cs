...
namespace ESimSol.Reports
{
    public class rptDUBatchCost
    {
        ...
        List<RSInQCDetail> _oRSInQCDetails = new List<RSInQCDetail>();
        ...
        #region
         public byte[] PrepareReport(DUBatchCost oDUBatchCost, List<DUBatchCost> oDUBatchCostDetail, List<RSRawLot> oRSRawLots, List<RSInQCDetail> oRSInQCDetails, Company oCompany, BusinessUnit oBusinessUnit,List<Lot> oLots)
        {
        ...
        private void ReporttHeader()
        {
            ...
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "BATCH COST", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, false, 0, _oFontStyleBold);
        ...
        private void SetRawMaterialUsed()
        {
            ...
            oPdfPTable.SetWidths(new float[] { 50f, 180f, 30f, 40f, 40f, 40f, 40f, 50f, 50f });
           ...
           _oDUBatchCostDetail = _oDUBatchCostDetail.OrderBy(o => o.ProductCategoryName).ToList();
            ...
            foreach (var oDUBatchCost in oDUBatchCosts)
            {
                ...
                {
                    //if (_oDUBatchCost.IsReDyeing == false)
                    //{
        ...
        private void SetRSInQCDetail()
        {
            _oFontStyle = FontFactory.GetFont("Tahoma", 8f, iTextSharp.text.Font.NORMAL);
            _oFontStyleBold = FontFactory.GetFont("Tahoma", 8f, iTextSharp.text.Font.BOLD);
            _oFontStyle_UnLine = FontFactory.GetFont("Tahoma", 10f, iTextSharp.text.Font.BOLDITALIC);
            PdfPTable oPdfPTable = new PdfPTable(5);
            oPdfPTable.SetWidths(new float[] {200f, 40f, 80f, 70f,  80f });

            #region PO Info
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Q/C Information", 0,5, Element.ALIGN_CENTER, BaseColor.GRAY, true, 0, _oFontStyle_UnLine);
            oPdfPTable.CompleteRow();
            ...
            _nQty_Recycle = _oRSInQCDetails.Where(x => x.YarnType == EnumYarnType.Recycle).Sum(P => P.Qty);

            _nTotalQty = _oRSRawLots.Sum(P => P.Qty);
            if (_nTotalQty > 0)
            {
                _nValue_Recycle = (_nValue_RawYarn / _nTotalQty) * _nQty_Recycle;
            }
            if (_oDUBatchCost.RSState == EnumRSState.LotReturn)
            {
                _nQty_Recycle = _nTotalQty;
                _nValue_Recycle = _nTotalAmount;
                _nValue_RawYarn = _nTotalAmount;
            }

            if (_oDUBatchCost.RSState != EnumRSState.LotReturn)
            {
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Fresh Yarn", 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, _oDUBatchCost.MUnit, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyleBold);
                _nTotalFreshYarn = _oRSInQCDetails.Where(x => x.YarnType == EnumYarnType.FreshDyedYarn).Sum(P => P.Qty);
                _nTotalFreshYarn = _nTotalFreshYarn + _oRSInQCDetails.Where(x => x.YarnType == EnumYarnType.DyedYarnOne || x.YarnType == EnumYarnType.DyedYarnTwo || x.YarnType == EnumYarnType.DyedYarnThree).Sum(P => P.Qty);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, Global.MillionFormat(_nTotalFreshYarn), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);

                ESimSolItexSharp.SetCellValue(ref oPdfPTable, ((_nTotalFreshYarn > 0) ? Global.MillionFormatActualDigit(Math.Round((_nTotalAmount - _nValue_Recycle) / _nTotalFreshYarn, 2)) : ""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, ((_nTotalFreshYarn > 0) ?  _oDUBatchCost.CurrencySymbol + "" + Global.MillionFormat(_nTotalAmount - _nValue_Recycle):""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
                oPdfPTable.CompleteRow();
            }
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Recycle Yarn", 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, _oDUBatchCost.MUnit, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyleBold);

            ESimSolItexSharp.SetCellValue(ref oPdfPTable,  ((_nQty_Recycle > 0) ?  Global.MillionFormat(_nQty_Recycle):""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, ((_nQty_Recycle > 0) ? Global.MillionFormatActualDigit(Math.Round(_nValue_RawYarn / _nTotalQty, 5)) : ""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, ((_nValue_Recycle > 0) ? _oDUBatchCost.CurrencySymbol + "" + Global.MillionFormatActualDigit(Math.Round(_nValue_Recycle, 5)) : ""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
            
            oPdfPTable.CompleteRow();

            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Wastage Yarn", 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, _oDUBatchCost.MUnit, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyleBold);
            _nTotalQty = _oRSInQCDetails.Where(x => x.YarnType == EnumYarnType.Wastage).Sum(P => P.Qty);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, ((_nTotalQty>0) ? Global.MillionFormat(_nTotalQty) : ""), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "", 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyle);
        ...
        private void SetBatchInfo()
        {
            ...
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Sale Rate(USD)", 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable,"$"+ Global.MillionFormat(_oDUBatchCost.UnitPrice), 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyleBold);
        ...
        private void SetRSCurrentLotStatus()
        {
            _oFontStyle = FontFactory.GetFont("Tahoma", 8f, iTextSharp.text.Font.NORMAL);
            _oFontStyleBold = FontFactory.GetFont("Tahoma", 8f, iTextSharp.text.Font.BOLD);
            _oFontStyle_UnLine = FontFactory.GetFont("Tahoma", 10f, iTextSharp.text.Font.BOLDITALIC);
            ...
            #region PO Info
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Current Status of Store ", 0, 5, Element.ALIGN_CENTER, BaseColor.GRAY, true, 0, _oFontStyle_UnLine);
            oPdfPTable.CompleteRow();
            ...
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Current Balance", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyleBold);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "U/P", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyleBold);
            ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Value", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyleBold);
            oPdfPTable.CompleteRow();
            ESimSolItexSharp.PushTableInCell(ref _oPdfPTable, oPdfPTable, 0, _oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, false, true);
            foreach (var oItem1 in _oLots)
            {
                oPdfPTable = new PdfPTable(5);
                oPdfPTable.SetWidths(new float[] { 200f, 40f, 80f, 70f, 80f });

                ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem1.OperationUnitName, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem1.LotNo, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyleBold);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, Global.MillionFormat(oItem1.Balance) + " " + oItem1.MUName, 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem1.Currency + "" + Global.MillionFormatActualDigit(oItem1.UnitPrice), 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem1.Currency + "" + Global.MillionFormat(oItem1.Balance * oItem1.UnitPrice), 0, 0, Element.ALIGN_RIGHT, BaseColor.WHITE, true, 0, _oFontStyle);
