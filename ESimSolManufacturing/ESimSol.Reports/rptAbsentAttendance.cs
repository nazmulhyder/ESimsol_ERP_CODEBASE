...
{
    public class rptAbsentAttendance
    {
        #region Declaration
        Document _oDocument;
        ...
        public byte[] PrepareReport(AttendanceDaily oAttendanceDaily)
        {
            ...
            {
                sHeader = "PRESENT EMPLOYEES OF " + _oAttendanceDailys[0].AttendanceDateInString;
            }

            else if (oAttendanceDaily.ErrorMessage == "Absent")
            {
                sHeader = "ABSENT EMPLOYEES OF " + _oAttendanceDailys[0].AttendanceDateInString;
            }

            else if (oAttendanceDaily.ErrorMessage == "Late")
            {
                sHeader = "LATE OF " + _oAttendanceDailys[0].AttendanceDateInString;
            }

            else if (oAttendanceDaily.ErrorMessage == "Early")
            {
                sHeader = "EARLY LEAVING OF " + _oAttendanceDailys[0].AttendanceDateInString;
            }
            ...
            _oDocument.Open();
            _oPdfPTable.SetWidths(new float[] { 20f, 70f, 90f, 100f, 45f, 50f, 50f, 50f, 35f });
            #endregion
        ...
        private void PrintBody()
        {
            ...
            if (_oAttendanceDailys.Count <= 0)
            {
                ...
                _oPdfPCell = new PdfPCell(new Phrase("There is no data to print !!", _oFontStyle)); _oPdfPCell.Colspan = 9; _oPdfPCell.Border = 0;
            ...
            {
                _oAttendanceDailys = _oAttendanceDailys.OrderBy(x => x.DepartmentName).ToList();
                while (_oAttendanceDailys.Count > 0)
                {
                    List<AttendanceDaily> oTempAttendanceDailys = new List<AttendanceDaily>();
                    oTempAttendanceDailys = _oAttendanceDailys.Where(x => x.DepartmentName == _oAttendanceDailys[0].DepartmentName).OrderBy(x => x.EmployeeCode).ToList();
                    //oTempDAs.AddRange(oTempAttendanceDailys);
                    PrintAttendanceDaily(oTempAttendanceDailys);
                    _oAttendanceDailys.RemoveAll(x => x.DepartmentName == oTempAttendanceDailys[0].DepartmentName);
                }
            }

            _oPdfPCell = new PdfPCell(new Phrase(" ", _oFontStyle)); _oPdfPCell.Colspan = 9; _oPdfPCell.Border = 0; _oPdfPCell.FixedHeight = 30;
            _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);
        ...
        {
            _oPdfPCell = new PdfPCell(new Phrase(" ", _oFontStyle)); _oPdfPCell.Colspan = 9; _oPdfPCell.Border = 0; _oPdfPCell.FixedHeight = 15;
            _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);
            _oPdfPTable.CompleteRow();

            _oFontStyle = FontFactory.GetFont("Arial Black", 8f, iTextSharp.text.Font.BOLD);
            _oPdfPCell = new PdfPCell(new Phrase("Department : " + oAD.DepartmentName, _oFontStyle)); _oPdfPCell.Colspan = 9; _oPdfPCell.Border = 0;
        ...
        private void PrintAttendanceDaily(List<AttendanceDaily> oADs)
        {
            ...
            foreach (AttendanceDaily oItem in oADs)
            {
                ...
                string sInTime = "";
                if (oItem.InTimeInString == "00:00")
                {
                    sInTime = "-";
                }
                else
                {
                    sInTime = oItem.InTimeInString;
                }

                _oPdfPCell = new PdfPCell(new Phrase(sInTime, _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);

                string sOutTime = "";
                if (oItem.OutTimeInString == "00:00")
                {
                    sOutTime = "-";
                }
                else
                {
                    sOutTime = oItem.OutTimeInString;
                }

                _oPdfPCell = new PdfPCell(new Phrase(sOutTime, _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);

                //string sLateArrivalMinute = "";
                //if (oItem.LateArrivalMinute <= 0)
                //{
                //    sLateArrivalMinute = "-";
                //}
                //else
                //{
                //    sLateArrivalMinute = oItem.LateArrivalMinute.ToString();
                //}

                _oPdfPCell = new PdfPCell(new Phrase(oItem.LateArrivalHourSt, _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);

                //string sEarlyDepartureMinute = "";
                //if (oItem.EarlyDepartureMinute <= 0)
                //{
                //    sEarlyDepartureMinute = "-";
                //}
                //else
                //{
                //    sEarlyDepartureMinute = oItem.EarlyDepartureMinute.ToString();
                //}
