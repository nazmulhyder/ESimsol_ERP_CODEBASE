        ...
        private void PrintBody()
        {
            ...
            if (_oPSDs.Count > 0)
            {
                ...
                foreach (ProductionScheduleDetail oItem in _oPSDs)
                {
                    ...
                    _oPdfPCell = new PdfPCell(new Phrase(oItem.UsesWeight+"", _oFontStyle));
                ...
                double nAvgShadePercentage = nAvgShadePercentage = (_oPSDs.Count(x => x.ShadePercentage > 0)>0)? _oPSDs.Sum(x => x.ShadePercentage) / _oPSDs.Count(x => x.ShadePercentage > 0):0;
