...
{
    public class rptPrintStatementForDeliveryLog
    {
        #region Declaration
        int _nTotalColumn = 1;
        Document _oDocument;
        ...
        public byte[] PrepareReport(FabricSalesContract oFabricSalesContract, List<FabricExpDelivery> oFabricExpDeliverys, Company oCompany)
        {
            _oFabricSalesContract = oFabricSalesContract;
            _oFabricExpDeliverys = oFabricExpDeliverys;
            _oCompany = oCompany;
        ...
        {
            this.PrintHeaderDetail("Statement Report", " ", " ");
            this.SetData();
        }
        #endregion
        ...
        private void SetData()
        {
            ...
            oPdfPTable.SetWidths(new float[] { 30f, 75f, 60f, 100f, 80f });

            

            var data = _oFabricExpDeliverys.GroupBy(x => new { x.FSCDID, x.ExeNo }, (key, grp) => new
            {
                FSCDID = key.FSCDID,
                ExeNo = key.ExeNo,
                DispoQty = grp.ToList().Select(y=>y.DispoQty).FirstOrDefault(),
                Results = grp.ToList()
            });
            ...
            foreach (var oData in data)
            {
                ...
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Dispo No: "+oData.ExeNo, 0, 3, Element.ALIGN_LEFT, BaseColor.WHITE, false, 0, _oFontStyle);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Dispo Qty: "+oData.DispoQty.ToString("#,##0.00;(#,##0.00)"), 0, 2, Element.ALIGN_LEFT, BaseColor.WHITE, false, 0, _oFontStyle);
                oPdfPTable.CompleteRow();
                ...
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Updated By", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyleBold);
                ESimSolItexSharp.SetCellValue(ref oPdfPTable, "Updated Date", 0, 0, Element.ALIGN_CENTER, BaseColor.WHITE, true, 0, _oFontStyleBold);
                oPdfPTable.CompleteRow();
                ESimSolItexSharp.PushTableInCell(ref _oPdfPTable, oPdfPTable, 0, _oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, false, true);
                #endregion
                ...
                foreach (var oItem in oData.Results)
                {
                    ...
                    ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem.DeliveryDateInString, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
                    ...
                    ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem.LastUpdateByName, 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
                    ESimSolItexSharp.SetCellValue(ref oPdfPTable, oItem.LastUpdateDateTime.ToString("dd MMM yyyy hh:mm tt"), 0, 0, Element.ALIGN_LEFT, BaseColor.WHITE, true, 0, _oFontStyle);
                    oPdfPTable.CompleteRow();
                    ESimSolItexSharp.PushTableInCell(ref _oPdfPTable, oPdfPTable, 0, _oPdfPTable.NumberOfColumns, Element.ALIGN_CENTER, BaseColor.WHITE, false, true);
                }
