...
{
    public class recordInactive
    {
        public string DepartmentName { get; set; }
        public int NoOfEmployee { get; set; }
    }
    public class rptDiscontinuedInactiveEmployeeWithSummery
    {
        #region Declaration
        ...
        List<recordInactive> showSummery = new List<recordInactive>();
        List<EmployeeActiveInactiveHistory> _oEmployeeActiveInactiveHistory = new List<EmployeeActiveInactiveHistory>();
        ...
        public byte[] PrepareReport(Employee oEmployee)
        {
            ...
            _oEmployeeActiveInactiveHistory = oEmployee.EmployeeActiveInactiveHistorys;
        ...
        private void PrintBody()
        {
            ...
            foreach (Department oDepartmentItem in _oDepartments)
            {
                RowCount++;
                RowCount++;
                recordInactive rec = new recordInactive();
                ...
                if (oEmployees.Count > 0)
                {
                    ...
                    foreach (Employee oEmployeeItem in oEmployees)
                    {
                        ...
                        if (_oEmployeeActiveInactiveHistory.Any(x => x.EmployeeID == oEmployeeItem.EmployeeID))
                        {
                            var obj = _oEmployeeActiveInactiveHistory.Where(x => x.EmployeeID == oEmployeeItem.EmployeeID).First();
                            _oPdfPCell = new PdfPCell(new Phrase(obj.InactiveDateInString, _oFontStyle));
                            _oPdfPCell.HorizontalAlignment = Element.ALIGN_LEFT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPTable.AddCell(_oPdfPCell);
        ...
        public void HeaderRow(Employee oEmployee)
        {
            ...
            _oPdfPCell = new PdfPCell(new Phrase("Inactive DATE", _oFontStyle));
            _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER; _oPdfPCell.BackgroundColor = BaseColor.LIGHT_GRAY; _oPdfPTable.AddCell(_oPdfPCell);
