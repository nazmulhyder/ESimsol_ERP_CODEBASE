...
{
    public class rptMultiMonthPaySlip
    {
        #region Declaration
        ...
        private void BodyReapeat(List<EmployeeSalary> oESItems)
        {
            foreach (EmployeeSalary oESItem in oESItems)
            {
                ...
                string sMonthYear = oESItem.StartDate.ToString("MMMM") + " " + oESItem.StartDate.Year + "  to  " + oESItem.EndDate.ToString("MMMM") + " " + oESItem.EndDate.Year;
                ...
                oTempESDetails = _oEmployeeSalaryDetails.Where(x => x.EmployeeID == oESItem.EmployeeID).OrderBy(x => x.SalaryHeadID).ToList();
