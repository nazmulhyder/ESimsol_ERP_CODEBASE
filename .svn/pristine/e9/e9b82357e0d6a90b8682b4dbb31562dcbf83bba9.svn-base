...
{
    public class rptProductionWithCostAnalysis
    {
        #region Declaration
        Document _oDocument;
        int _nCloumns = 13;
        ...
        #endregion
        #region Constructor
        public rptProductionWithCostAnalysis() { }
        #endregion
        ...
        private void PrintBody()
        {
            ...
            double nTotalAvgAddShadePercentage = 0;
            ...
            if (nTotalElement > 0)
            {
                ...
                nTotalAvgCostPerKg = Math.Round(_oRPCAs.Sum(x => Math.Round(x.TotalCost)) / (_oRPCAs.Sum(x => Math.Round(x.ProductionQty))>0?_oRPCAs.Sum(x => Math.Round(x.ProductionQty)) :1) );
                nTotalAvgBatchPerMachine = Convert.ToDouble(_oRPCAs.Count()) / Convert.ToDouble(_oRPCAs.Select(x => x.MachineName).Distinct().Count());
               
                nTotalAvgBatchLoading = Convert.ToDouble(_oRPCAs.Sum(x => Math.Round(x.ProductionQty)) * 100 / (_oRPCAs.Sum(x => x.UsesWeight)>0?_oRPCAs.Sum(x => x.UsesWeight) :1)  );                   
                nTotalAvgShadePercentage = Math.Round(Convert.ToDouble((_oRPCAs.Count(x => x.DyesQty > 0) > 0) ? _oRPCAs.Where(x => x.DyesQty > 0).Sum(x => x.TotalDyesQty) * 100 /( _oRPCAs.Sum(x => Math.Round(x.ProductionQty))>0? _oRPCAs.Sum(x => Math.Round(x.ProductionQty)) :1)  : 0), 2);
                nTotalAvgAddShadePercentage = (_oRPCAs.Count(x => x.Remark != "Ok") > 0) ? _oRPCAs.Where(x => x.Remark != "Ok").Sum(x => x.AdditionalPercentage) / (_oRPCAs.Count(x => x.DyesCost > 0)>0?Convert.ToDouble(_oRPCAs.Count(x => x.DyesCost > 0)) :1)   : 0;


                nTotalAvgNetRFT = _oRPCAs.Count(x => x.Remark == "Ok") > 0 ? (_oRPCAs.Count(x => x.Remark == "Ok") * 100) / (_oRPCAs.Count()>0?Convert.ToDouble(_oRPCAs.Count()) :1) : 0;
                ...
                {
                    nTotalAvgGrossRFT = nTotalAvgNetRFT + ((nAddOneCount * 65) + (nAddTwoCount * 35) + (nAddThreeCount * 0)) / (_oRPCAs.Count()>0?_oRPCAs.Count() :1);
                }
                ...
                while (_oRPCAs.Count() > 0)
                {
                    ...
                    if (oRPCAs.Count()>0)
                    {
                        ...
                        //nTotalAvgBatchLoading = nTotalAvgBatchLoading + Convert.ToDouble(oRPCAs.Sum(x => Math.Round(x.ProductionQty)) * 100 / oRPCAs.Sum(x => x.UsesWeight));                   
                        ...
                        if (nAddition > 0) { nAvgAddition = oRPCAs.Where(x => x.Remark != "Ok").Sum(x => x.AdditionalPercentage) / Convert.ToDouble(oRPCAs.Count(x => x.DyesCost > 0)); }

                        //nTotalAvgAddShadePercentage = nTotalAvgAddShadePercentage + nAvgAddition;
            ...
            _oPdfPCell = new PdfPCell(new Phrase("Total DC Cost(BDT): " + Global.MillionFormat(nTotalDCCost), _oFontStyle));
            _oPdfPCell.Colspan = 4; _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPCell.Border = 0; _oPdfPTable.AddCell(_oPdfPCell);
            ...
            _oPdfPCell = new PdfPCell(new Phrase("Cost Per kg(BDT): " + Global.MillionFormat(nTotalAvgCostPerKg), _oFontStyle));
            _oPdfPCell.Colspan = 4; _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPCell.Border = 0; _oPdfPTable.AddCell(_oPdfPCell);
            ...
            _oPdfPCell = new PdfPCell(new Phrase("Avg. Addition Shade(%): " + Math.Round(nTotalAvgAddShadePercentage, 2) + "%", _oFontStyle));
            _oPdfPCell.Colspan = 4; _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPCell.Border = 0; _oPdfPTable.AddCell(_oPdfPCell);
