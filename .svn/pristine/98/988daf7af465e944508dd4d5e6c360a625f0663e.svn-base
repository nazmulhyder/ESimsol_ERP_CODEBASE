...
{
    public class rptProductionSummaryByMachine
    {
        #region Declaration
        Document _oDocument;
        int _nCloumns = 10;
        ...
        #region Constructor
        public rptProductionSummaryByMachine() { }
        #endregion
        ...
        public byte[] PrepareReport(List<RptProductionCostAnalysis> oRPCAs, Company oCompany, string sValue)
        {
            ...
            _oPdfPTable.SetWidths(new float[] { 30f, 50f, 75f, 80f, 95f, 35f, 45f, 45f, 35f, 65f }); //height:842   width:595
        ...
        private void PrintHeader()
        {
            ...
            if (_oRPCAs.Count() > 0)
            {
                _oPdfPCell = new PdfPCell(new Phrase(_oRPCAs[0].MachineName, _oFontStyle));
                _oPdfPCell.Colspan = _nCloumns;
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
                _oPdfPCell.Border = 0;
                _oPdfPCell.BackgroundColor = BaseColor.WHITE;
        ...
        private void PrintBody()
        {
            ...
            if (_oRPCAs.Count > 0)
            {
                ...
                foreach (RptProductionCostAnalysis oItem in _oRPCAs)
                {
                    ...
                    _oPdfPCell = new PdfPCell(new Phrase(Global.MillionFormat(Math.Round(oItem.ProductionQty)).Split('.')[0], _oFontStyle));
                ...
                nTotalBatchLoading = Convert.ToDouble(_oRPCAs.Sum(x => x.ProductionQty) * 100 / _oRPCAs.Sum(x => x.UsesWeight));
                ...
                _oPdfPCell = new PdfPCell(new Phrase("Avg Time: " + TimeStampCoversion(nTotalDay, nTotalHour, nTotalMin, _oRPCAs.Count()), _oFontStyle));
                ...
                _oPdfPCell = new PdfPCell(new Phrase("Batch Loading: " + Math.Round(nTotalBatchLoading, 2) + "%", _oFontStyle));
                _oPdfPCell.Colspan = 9; _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPCell.Border = 0; _oPdfPTable.AddCell(_oPdfPCell);
                _oPdfPCell = new PdfPCell(new Phrase("", _oFontStyle));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; _oPdfPCell.Border = 0; _oPdfPTable.AddCell(_oPdfPCell);
                _oPdfPTable.CompleteRow();
