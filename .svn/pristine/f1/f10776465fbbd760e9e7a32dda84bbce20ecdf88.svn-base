...
{
    public class rptProductDetailsStockQty
    {
        #region Declaration
        Document _oDocument;
        ...
        public byte[] PrepareProductWiseReport(List<Lot> oLots, Company oCompany, BusinessUnit oBusinessUnit, int layout)
        {
            _oLots = oLots;
            _oCompany = oCompany;
            ...
            this.PrintHeader(layout==(int)EnumStockReportLayout.ProductWise? "Product Wise Stock Report":"Product & Color Wise Stock Report");
        ...
        {
            int nWidth = (layout == (int)EnumStockReportLayout.ProductWise ? 5 : 6);
            PdfPTable oPdfPTable = new PdfPTable(nWidth);
            float[] dynamicAraray = new float[nWidth];
            dynamicAraray[0] = 30f;//sL
            dynamicAraray[1] = 55f;//code
            dynamicAraray[2] = 232f;//Product name
            for (int i = 3; i < nWidth;i++ )
            {
                dynamicAraray[i] = 228/(nWidth-3);//sL
            }
            ...
            if (layout == (int)EnumStockReportLayout.ProductAndColorWise)
            {
            ...
            int nCount = 0; double nStockQtyTotal= 0;
            ...
            foreach (Lot oItem in _oLots)
            {
                ...
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);
                nStockQtyTotal = nStockQtyTotal + oItem.Balance;
                oPdfPTable.CompleteRow();
            }
            ...
            _oPdfPCell.Colspan = (layout == (int)EnumStockReportLayout.ProductWise ? 3 : 4); _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);
                        
            _oPdfPCell = new PdfPCell(new Phrase(Global.TakaFormat(nStockQtyTotal), _oFontStyle));
            _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.WHITE; oPdfPTable.AddCell(_oPdfPCell);
