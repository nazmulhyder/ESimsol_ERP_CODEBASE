var IICService = angular.module('DUSoftWindingReport.service', ['ms.service', 'ngAnimate', 'ui.bootstrap']);
IICService.service('DUSoftWindingReportservice', function ($uibModal) {
    ...
    this.Instance = function (modalProperty) {
        ...
        var modalInstance = {
            ...
            size: 'md',
            templateUrl: _sBaseAddress + '/DUSoftWinding/AdvSearchDUSoftWinding_Report',
        ...
        modalInstance.controller = function ($http, $scope, $log, $uibModalInstance, uiGridConstants, msModal, icsMethod, obj) {
            ...
            $scope.PickLot = function () {
                var oLot = {
                    LotNo: $.trim($scope.LotNo)
                };
                var config = { headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;' } };
                $http.post(_sBaseAddress + '/DUSoftWinding/GetLots_Rpt', $.param(oLot), config).then(
                                    function (response) {
                                        var oColumns = [];

                                        var oColumn = { field: 'LotNo', name: 'Lot No', width: '20%', enableSorting: false }; oColumns.push(oColumn);
                                        oColumn = { field: 'ProductName', name: 'ProductName', width: '40%', enableSorting: false }; oColumns.push(oColumn);
                                        oColumn = { field: 'BalanceSt', name: 'Balance', width: '40%', enableSorting: false }; oColumns.push(oColumn);
                                        oColumn = { field: 'OperationUnitName', name: 'Store', width: '40%', enableSorting: false }; oColumns.push(oColumn);
                                        var results = JSON.parse(response.data);
                                        var modalObj = {
                                            size: 'md',
                                            modalcontroller: 'ModalCommonListCtrl',
                                            appcontroller: 'LotController',
                                            objs: results,
                                            multiSelect: true,
                                        ...
                                        modalInstance.result.then(function (result) {
                                            ...
                                            if (result.length > 0) {
                                                ...
                                                $scope.LotIDs = result[0].LotID;
                                                for (var i = 1; i < result.length; i++) {
                                                    $scope.LotIDs += "," + result[i].LotID;
                                                }
                                                if (result.length > 1) $scope.LotNo = result.length + " Items are selected.";
                                            }
                                        }, function () {
                                            $log.info('Modal dismissed at: ' + new Date());
                                        });
            ...
            $scope.search = function () {
                ...
                if ($scope.OrderNo == "" && $scope.Contractor.BuyerIDs == "" && $scope.LotIDs == "" && $scope.cboOrderDate == 0 && $scope.cboIssueDate == 0 && $scope.cboReceiveDate == 0 && $scope.YetToApprove == false && $scope.YetToIssue == false && $scope.YetToReceive == false) {
                ...
                var sTempString = $scope.cboOrderDate + '~' + $scope.OrderDateStart + '~' + $scope.OrderDateEnd + '~'
                                ...
                                + $scope.cboIssueDate + '~' + $scope.IssueDateStart + '~' + $scope.IssueDateEnd + '~'
                                + ($scope.OrderNo == undefined ? "" : $scope.OrderNo) + '~' + ($scope.LotIDs == undefined ? "" : $scope.LotIDs) + '~'
                ...
                var config = { headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;' } };
                $http.get(_sBaseAddress + '/DUSoftWinding/AdvSearch_Rpt', { params: { sTemp: sTempString } }, config).then(
                            function (response) {
                                debugger;
