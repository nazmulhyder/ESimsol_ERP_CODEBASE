...
namespace ESimSol.Reports
{
    public class rptBankSheetV2
    {
        ...
        List<EmployeeSalaryV2> _oEmployeeSalarys = new List<EmployeeSalaryV2>();
        ...
        bool isRound = false,_isOT=false;
        string _dateHeader = "";
        ...
        private void PrintBankPart()
        {
            ...
            double nAmtWithOT = _oEmployeeSalarys.Sum(x => x.BankAmount);
            if(_isOT)
            {
                nAmtWithOT += _oEmployeeSalarys.Sum(x => x.TotalOT);
            }
            Chunk oChunk2 = new Chunk(nAmtWithOT.ToString("###,##0;(###,##0)"), _oFontStyleBold); oPhrase.Add(oChunk2);
            Chunk oChunk3 = new Chunk(" (In Words : ", _oFontStyle); oPhrase.Add(oChunk3);
            Chunk oChunk4 = new Chunk(Global.TakaWords(Math.Round(nAmtWithOT, 0)), _oFontStyleBold); oPhrase.Add(oChunk4);
        ...
        private void PrintBody()
        {
            ...
            foreach (EmployeeSalaryV2 oItem in _oEmployeeSalarys)
            {
                ...
                if (oItem.BankAmount > 0)
                {
                    ...
                    if (matchString != null)
                    {
                        _oPdfPCell = new PdfPCell(new Phrase(oItem.JoiningDate.ToString("dd MMM yyyy"), _oFontStyle));
                    ...
                    if (matchString != null)
                    {
                        double nOTAllowance = 0;
                        if(_isOT)
                        {
                            nOTAllowance = Math.Round(oItem.TotalOT);
                        }

                        nTotalAmount += oItem.BankAmount + nOTAllowance;
                        _oPdfPCell = new PdfPCell(new Phrase((isRound == true) ? Math.Round((oItem.BankAmount + nOTAllowance), 0).ToString("###,##0;(#,##,##)") : (oItem.BankAmount + nOTAllowance).ToString("###,##0;(#,##,##0)"), _oFontStyle));
            ...
            if (matchString != null)
            {
                ...
                double nAmountWithOT =_oEmployeeSalarys.Sum(x => x.BankAmount);
                if(_isOT)
                {
                    nAmountWithOT += _oEmployeeSalarys.Sum(x => x.TotalOT);
                }
                _oPdfPCell = new PdfPCell(new Phrase(Math.Round(nAmountWithOT, 2).ToString("###,##0;(###,##0)"), _oFontStyleBold));
                _oPdfPCell.HorizontalAlignment = Element.ALIGN_RIGHT; _oPdfPCell.BackgroundColor = BaseColor.LIGHT_GRAY; oPPdfPTable.AddCell(_oPdfPCell);
                ...
                _oPdfPCell = new PdfPCell(new Phrase("In Words : " + Global.TakaWords(Math.Round(nAmountWithOT, 0)), _oFontStyleBold)); _oPdfPCell.Colspan = _selectedColNames.Length + 1;
                _oPdfPCell.Border = 0;
