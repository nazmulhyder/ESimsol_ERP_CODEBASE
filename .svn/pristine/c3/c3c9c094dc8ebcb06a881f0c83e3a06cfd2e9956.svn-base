...
advanceSearchservice.service('advanceSearchservice', function ($uibModal) {
    ...
    this.Instance = function (modalProperty) {
        var modalInstance = {
            ...
            size: 'md',
            templateUrl: sessionStorage.getItem('BaseAddress') + '/VehicleEngine/AdvanceSearch',
        ...
        modalInstance.controller = function ($http, $scope, $log, $uibModalInstance, uiGridConstants, msModal, icsMethod, obj) {
            ...
            $scope.PickerTitle = "Vehicle Engine Search";
            ...
            $scope.search = function () {
                ...
                if (($scope.ManufacturerName == undefined ? "" : $scope.ManufacturerName) && ($scope.Cylinders == undefined ? "" : $scope.Cylinders) &&
                   ($scope.EngineCoolant == undefined ? "" : $scope.EngineCoolant) && ($scope.CompressionRatio == undefined ? "" : $scope.CompressionRatio) &&
                   ($scope.ManufacturerIDs == undefined ? "" : $scope.ManufacturerIDs) == "" && ($scope.FileNo == undefined ? "" : $scope.FileNo) == "" &&
                   ($scope.CatalyticConverter == undefined ? "" : $scope.CatalyticConverter) == "" && ($scope.EngineNo == undefined ? "" : $scope.EngineNo) == "") {
                    alert("Please Select at least one Criteria !!");
                    return;
                }
                debugger;
                var sTempString = ($scope.FileNo == undefined ? "" : $scope.FileNo) + '~' + ($scope.EngineNo == undefined ? "" : $scope.EngineNo) + '~'
                                + ($scope.ManufacturerIDs == undefined ? "" : $scope.ManufacturerIDs) + '~' + ($scope.Cylinders == undefined ? "" : $scope.Cylinders) + '~' + ($scope.CompressionRatio == undefined ? "" : $scope.CompressionRatio) + '~'
                                + ($scope.BMEP == undefined ? "" : $scope.BMEP) + '~' + ($scope.EngineCoolant == undefined ? "" : $scope.EngineCoolant) + '~' + ($scope.CatalyticConverter == undefined ? "" : $scope.CatalyticConverter);

                var config = { headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;' } };
                $http.get(sessionStorage.getItem('BaseAddress') + '/VehicleEngine/Search', { params: { sTemp: sTempString } }, config).then(
                            function (response) {
                                debugger;
                                $scope.lblLoadingMessage = true;
                                var results = jQuery.parseJSON(response.data);
            ...
            $scope.reset = function () {
                $scope.EngineNo = $scope.FileNo = $scope.ManufacturerName = $scope.Cylinders = $scope.BMEP = $scope.CatalyticConverter = '';
                $scope.ManufacturerIDs = "";
            }
            $scope.reset();
